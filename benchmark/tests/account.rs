...
#[test]
fn test_account_with_bincode_v1() {
    let mollusk = setup(&PROGRAM_ID, "bincode_v1_program");
    ...
    let account: Account = deserialize(data).unwrap();
...
#[test]
fn test_account_with_bincode_v2() {
    let mollusk = setup(&PROGRAM_ID, "bincode_v2_program");
    ...
    let (account, _): (Account, usize) =
        borrow_decode_from_slice(data, standard().with_fixed_int_encoding()).unwrap();
